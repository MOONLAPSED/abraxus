...
def main():
    ...
    root_ns.frame_model = ConcreteFrameModel("{{", "}}")
...
class FrameModel(Generic[T, V, C], ABC):
    ...
    def validate_content(self, content: str) -> bool:
        ...
        if not content.startswith(self.start_delimiter) or not content.endswith(self.end_delimiter):
            return False
        return True
class ConcreteFrameModel(FrameModel[T, V, C]):
    def to_bytes(self) -> bytes:
        # Provide a concrete implementation
        content = f"{self.start_delimiter}...{self.end_delimiter}"
        return content.encode('utf-8')

    def parse_content(self, raw_content: str) -> str:
        # Provide a concrete implementation
        if self.validate_content(raw_content):
            # Extract content between delimiters
            start_len = len(self.start_delimiter)
            end_len = len(self.end_delimiter)
            return raw_content[start_len:-end_len]
        else:
